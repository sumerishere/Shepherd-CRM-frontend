...
const LeadFollowUp = () => {
  ...
  const [editCommentId, setEditCommentId] = useState(null); // Add state for the comment being edited
  const [editCommentText, setEditCommentText] = useState(""); // Add state for the edited comment text
  ...
  const handleCommentUpdate = async (commentId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/update-comment?commentId=${commentId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        const updatedComments = historyData.map((item) =>
          item.id === commentId
            ? { ...item, comment: editCommentText, updatedAt: new Date() }
            : item
        );
        setHistoryData(updatedComments);
  ...
  if (leads.length === 0) {
    return (
      <div className="follow-up-div">
        ...
        <p id="client-data-empty-text">No entries are available.</p>
        <Link to="/add-lead">
          <button id="add-lead-button">Add New Lead</button>
        </Link>
      </div>
    );
  } else {
                  ...
                  <div key={item.id} className="history-comment-item">
                    {editCommentId === item.id ? (
                      <div className="edit-comment">
                        ...
                        />
                        <button onClick={() => handleCommentUpdate(item.id)}>
                          Update
                        </button>
